...
void drawDigitalClockFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    // uint16_t hourMinuteTextY = (display->getHeight() / 2) - (((segmentWidth * 2) + (segmentHeight * 3) + 8) / 2);
     uint16_t hourMinuteTextY = (display->getHeight() / 2) - (((segmentWidth * 2) + (segmentHeight * 3) + 8) / 2) - topLift;
...
#endif
    if (config.display.use_12h_clock) {
        if (yOffset + FONT_HEIGHT_SMALL > usableHeight)
            return;
    ...
    //                     secondString);
#endif
graphics::drawCommonHeader(display, x, y, "Clock");
}
...
void drawAnalogClockFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    int16_t centerX = display->getWidth() / 2;
    int16_t centerY = (display->getHeight() / 2) - topLift;
